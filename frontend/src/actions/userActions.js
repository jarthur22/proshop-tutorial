...
const updateUserProfile = (user) => (dispatch, getState) => {
    ...
    axios.put(`/api/users/profile`, user, config).then(user => {
        dispatch({type: 'USER_UPDATE_PROFILE_SUCCESS', payload: user.data});
    }).catch(err => {
        dispatch({
